...
class ContextStore:
    ...
    def write_agent_bundle_examples(self) -> None:
        examples = (
            "# Agent Brief Bundle Integration Examples\n\n"
            "Copy one of the snippets below into your agent brief files so every agent reads "
            "`context/artifact_reference_bundle.md` before making Docker-related decisions.\n\n"
            "## Example for `context/main_agent_brief.md`\n\n"
            "```md\n"
            ...
            "```\n\n"
            "## Example for `context/docker_agent_brief.md`\n\n"
            "```md\n"
            "## Required Inputs\n\n"
            ...
            "- When revising the Dockerfile, explain the fix in `reports/docker_report.md` compatible terms.\n"
            "```\n\n"
            "## Example for `context/validator_agent_brief.md`\n\n"
            "```md\n"
            "## Validation Intake\n\n"
            "- Read `context/artifact_reference_bundle.md` before running checks.\n"
            "- Compare the current `Dockerfile` with the latest changed files listed in the bundle.\n"
            "- On failure, return concrete summaries that help the Docker agent revise the image definition.\n"
            "```\n"
        )
